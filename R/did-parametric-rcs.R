...
did_parametric_repeatedCS <- function(data, x_colnames) {
  ...
  for (tt in 1:length(data)) {
    ...
    objective <- Minimize(sum((data[[tt]]$Y - Xbind %*% beta_coef)^2))  
    ...
    diff_mat <- rbind(time_pre)
    
    for (m in 1:moment_vec) {
      ## get differencing operator mat (see genlasso)
      Dmat <- getDtf(time_pre, m-1)
...
gen_dummies <- function(data, add_intercept = TRUE) {
  ...
  group_dummy <- model.matrix(~ treatment - 1, data = pdata)
  ## generate time dummies 
  time_dummy  <- model.matrix(~ id_time - 1, data = pdata)
  ## generate interaction dummies 
  interact_dummy <- model.matrix(~ treatment:id_time - 1, data = pdata)
  
  if(isTRUE(add_intercept)) {
    ones <- rep(1, nrow(pdata))
    Xmat <- cbind(ones, group_dummy, time_dummy, interact_dummy)
  } else {
